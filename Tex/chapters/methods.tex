...
\label{ssec:theory}
    ...
    \begin{equation}
        \beta = \frac{1}{k_B T}
    \end{equation}
    Further the free energy \(F\) of a canonical
    ...
    So \(U\) is low if all spins are aligned, while \(S\) as a measure of
    ...
    %~ Once one has obtained values for e.g. the magnetization \(m\), one
    %~ can easily calculate the susceptibility
    %~ \begin{equation}
        %~ \chi = \frac{N}{T}\brac{\avg{m^2}-\avg{m}^2}
    %~ \end{equation}
    % Bisschen erklæren, dass man aus m auch chi machen kann und auf literatur verweisen
...
\label{ssec:montecarlo}
    ...
    \label{sssec:eqtime}
        ...
        and waits until it reaches thermal \emph{equilibrium}. Thermal
        equilibrium denotes the condition, that the system under scrutiny
        has reached the temperature of the heat bath. This is the case,
        once no energy is exchanged between them, such that the energy
        of the simulated system reaches a steady state as pictured in
        fig. \ref{fig:equiandauto}\subref{sfig:equiandauto:autoM}.
