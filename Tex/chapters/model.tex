...
\label{ssec:finitesize}
    ...
    of the disordered Ising model in dependece of the disorder parameter
    \(\sigma\). At \(T_c\) the magnetisation \(|m|\) of the system will
    jump and the susceptibility \(\chi_m = \frac{N}{T}\brac{\avg{m^2}-\avg{m}^2}\)
    will diverge. But this behaivior is only present in infinite systems,
    hence every computer simulation will show some \emph{finte size effects}.
    Because of this all through computer simulations observed phase
    transitions will be "smeared out".
    [todo: Bezug zum Bild]
    \begin{figure}[htbp]
        \centering
        \subfigure[todo][]
    ...
    In fig. \ref{fig:gettingCrit}\subref{sfig:gettingCrit:binder_fit_s_0}
    such an interpolation is plotted for \(\sigma=0\). The cumulants
    intersect at \(\approx 2.27\).
