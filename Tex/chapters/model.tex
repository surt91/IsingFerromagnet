...
\label{ssec:isingmodel}
    ...
    For the standard Ising model \(J_{ij}=1 \forall i,j\) applies.
    \(\tilde{H}_i\) denotes the outer magnetic field at the position of
    site \(i\). For the standard Ising model \(\tilde{H}_i=0 \ \forall i\)
    applies.\\
    The most important modification of the standard Ising modell in this
    ...
    standard Ising model. For this exists an analytic solution \cite{Onsager1944} for the
...
\label{ssec:graphtypes}
    ...
    from the Sec.\ \ref{ssec:isingmodel}.
