...
def getConfig():
    ...
    c16  = [(s,  realisierungUnordnung[16],  sweeps[16],  16,  t16 ,  equilibrium[16],  autokorr[16]) for s in sigmas]
    c32  = [(s,  realisierungUnordnung[32],  sweeps[32],  32,  t32l,  equilibrium[32],  autokorr[32]) for s in sigmas if 0.0<=s<0.3] \
         + [(s,  realisierungUnordnung[32],  sweeps[32],  32,  t32h,  equilibrium[32],  autokorr[32]) for s in sigmas if 0.3<=s<1.3]
    c64  = [(s,  realisierungUnordnung[64],  sweeps[64],  64,  t64l,  equilibrium[64],  autokorr[64]) for s in sigmas if 0.0<=s<0.3] \
