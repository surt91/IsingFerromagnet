...
def getConfig():
    ...
    t256h=",".join([str(i) for i in arange(0.1,0.55,0.06)]+[str(i) for i in arange(0.55,1.0,0.009)]+[str(i) for i in arange(1.0,1.5,0.003)]+[str(i) for i in arange(1.5,1.8,0.01)]+[str(i) for i in arange(1.8,2.08,0.010)]+[str(i) for i in arange(2.08,2.44,0.01)]+[str(i) for i in arange(2.44,3.3,0.02)]) #passend fur 256

    equilibrium = {16:20*2, 32:50*2, 64:50*2, 128:100*2, 256:300*2}
    autokorr    = {16: 3*2, 32: 5*2, 64: 7*2, 128: 12*2, 256: 16*2}

    c16  = [(s,  16,  t16 ,  equilibrium[16],  autokorr[16]) for s in arange(0.0,1.3,0.1)]
    c32  = [(s,  32,  t32l,  equilibrium[32],  autokorr[32]) for s in arange(0.0,0.3,0.1)] \
         + [(s,  32,  t32h,  equilibrium[32],  autokorr[32]) for s in arange(0.3,1.3,0.1)]
    c64  = [(s,  64,  t64l,  equilibrium[64],  autokorr[64]) for s in arange(0.0,0.3,0.1)] \
         + [(s,  64,  t64h,  equilibrium[64],  autokorr[64]) for s in arange(0.3,1.3,0.1)]
    c128 = [(s, 128, t128l, equilibrium[128], autokorr[128]) for s in arange(0.0,0.2,0.1)] \
         + [(s, 128, t128m, equilibrium[128], autokorr[128]) for s in arange(0.2,0.3,0.1)] \
         + [(s, 128, t128h, equilibrium[128], autokorr[128]) for s in arange(0.3,1.3,0.1)]
    c256 = [(s, 256, t256l, equilibrium[256], autokorr[256]) for s in arange(0.0,0.2,0.1)] \
         + [(s, 256, t256m, equilibrium[256], autokorr[256]) for s in arange(0.2,0.3,0.1)] \
         + [(s, 256, t256h, equilibrium[256], autokorr[256]) for s in arange(0.3,1.3,0.1)]
